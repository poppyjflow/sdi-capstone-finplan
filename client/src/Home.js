...
const getQuarterliesFilterValue = (mainData) => {
  return [
    {
      name: 'mainData.id',
      value: mainData ? mainData.id : '',
      operator: 'eq',
      type: 'string'
    }
    // ,
...
const Home = () => {
  ...
  // dataSource
  const getDataSource = (whichData) => {
    var rslt = [];
    return ({ skip, limit, sortInfo, groupBy, filterValue }) => {
      skip=null; limit=null; sortInfo=null; groupBy=null;
      if (filterValue && filterValue.length) {
//        console.log(`filterValue: ${filterValue[0].value}`);
        switch (whichData) {
          case 'q1':
            rslt = dataSource.filter(row => (row.id === filterValue[0].value)).map(row => ({ id: row.id, desc: row.q1_desc, q1_requested: row.q1_requested, q1_allocated: row.q1_allocated, q1_obligated: row.q1_obligated }));
            break;
          case 'q2':
            rslt = dataSource.filter(row => (row.id === filterValue[0].value)).map(row => ({ id: row.id, desc: row.q2_desc, q2_requested: row.q2_requested, q2_allocated: row.q2_allocated, q2_obligated: row.q2_obligated }));
            break;
          case 'q3':
            rslt = dataSource.filter(row => (row.id === filterValue[0].value)).map(row => ({ id: row.id, desc: row.q3_desc, q3_requested: row.q3_requested, q3_allocated: row.q3_allocated, q3_obligated: row.q3_obligated }));
            break;
          case 'q4':
            rslt = dataSource.filter(row => (row.id === filterValue[0].value)).map(row => ({ id: row.id, desc: row.q4_desc, q4_requested: row.q4_requested, q4_allocated: row.q4_allocated, q4_obligated: row.q4_obligated }));
            break;
       }
...
const gridStyleExpanded = { minHeight: 80 };
...
const q1DataSource = useCallback(getDataSource('q1'), []);
const q2DataSource = useCallback(getDataSource('q2'), []);
const q3DataSource = useCallback(getDataSource('q3'), []);
const q4DataSource = useCallback(getDataSource('q4'), []);
// Generate the expand/collapsible "+/-" rows showing the quarterly columns.
const renderQuarterliesGrid = ({ data }) => {
  const defaultFilterValue = getQuarterliesFilterValue(data);

  return (
    <div>
      <ReactDataGrid
        style={gridStyleExpanded}
